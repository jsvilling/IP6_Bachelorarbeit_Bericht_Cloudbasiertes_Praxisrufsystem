...
Dies beinhaltet insbesondere die Anbindung der API des Cloudservice und Firebase Cloud Messaging.
...
Bei Anrufen wird beschrieben, ob es sich um einen empfangenen, verpassten oder abgelehnten Anruf handelt.
...
Der Sender wird nicht über die Quittierung informiert~\cite{ip5}.
...
Es wird eine Anbindung an das Modul Configuration zur Anmeldung und Auswahl der gewünschten Konfiguration und an das Modul Notification zum Versenden von Benachrichtigungen benötigt.
...
Die Methode post nimmt zudem einen optionalen Parameter von Typ Data entgegen.
...
Dadurch kann die Konvertierung generisch im PraxisurfApi-Service behandelt werden.
...
Um die Verwendung der Cloudservice API weiter zu vereinfachen, wird der PraxisrufApi-Service um spezifische Methoden für benötigte Abfragen erweitert.
...
Dazu werden Service-Klassen implementiert, welche den Aufruf des PraxisrufAPI-Services kapseln.
Dieses Services bieten Methoden, über welche der PraxisrufApi-Service angesprochen werden kann.
...
Die View-Komponenten können auf diese Variablen zugreifen, um die entsprechenden Resultate oder Fehler anzuzeigen.

\subsubsection{Anbindung Messaging Service}

Um Benachrichtigungen empfangen zu können, muss Firebase Cloud Messaging (FCM) an die native iOS Applikation angebunden werden.
...
Dies liegt daran, dass das Empfangen von Push-Benachrichtigungen und die Registrierung bei FCM nur über AppDelegates umgesetzt werden kann~\cite{firebase_ios}.
...
Eine Ausnahme bildet die Anfrage, um sich im System anzumelden.
