...
Diese sind werden mit den Komponenten CallService und PraxisrufApi+Signaling implementiert.
...
Nachdem die Verbindung hergestellt wurde, werden im Abstand von 60 Sekunden Pingmeldungen über die Verbindung gesendet.
...
Zudem wird ein Fehlerdialog in der Benutzeroberfläche angezeigt.
Die Verbindung bleibt geschlossen bis die nächste Meldung versendet wird.
Dies der Fall, wenn das Interval für die nächste Pingmeldung abgelaufen ist und wenn ein Anruf über die Benutzeroberfläche gestartet wird.

Dieser Ablauf stellt sicher, dass die Verbindung zu der Signaling Instanz automatisch repariert wird.
Wenn möglich, geschieht dies direkt nach Verbindungsverlust.
Andernfalls werden Praxismitarbeitende informiert, dass die Gegensprechanlage aktuell nicht verwendet werden kann.
Im Hintergrund in dieser Situation im Abstand von 60 Sekunden versucht, die Verbindung zu reparieren.
Weiter wird die Verbindung repariert, wenn Praxismitarbeitende die Gegensprechanlage bedienen während die Verbindung getrennt ist.
In diesem Fall kann entweder die Verbindung wiederhergestellt und die Gegensprechanlage verwendet werden oder es wird erneut der Fehlerdialog angezeigt.
